...
import com.github.emd.myutils.io.FilesEx
...
class HexDumper(settings: HexDumper.Settings) {
  ...
  protected val decoder: CharsetDecoder =
    settings.charset.newDecoder().replaceWith(".")

  /** How many bytes per section in hexadecimal view. */
  protected val hexSectionBytes: Int =
    HexDumper.hexSectionBytes(settings.hexViewMode)
  ...
  protected val hexLastSectionSize: Int = computeHexSectionSize(
    hexLastSectionBytes
  )
  ...
  protected def processHexSection(section: BytesView): String = {
    ...
    settings.hexViewMode match {
      ...
      case HexadecimalViewMode.Compact =>
        bytes.grouped(2).map(_.mkString("")).mkString(" ")
    }
  }
  ...
  protected def processHexadecimal(
      data: Array[Byte],
      dataOffset: Int,
      dataLength: Int
  ): Unit = {
    ...
    sectionsWithSize.foldLeft(0) { case (index, (section, sectionSize)) =>
      ...
      settings.output.append(
        ensureLength(processHexSection(section), sectionSize)
      )
      index + 1
    }
    ()
  ...
  protected def processAscii(
      data: Array[Byte],
      dataOffset: Int,
      dataLength: Int
  ): Unit = {
    // Filter ISO-8859 non-visible characters
    val filtered = data.view
      .slice(dataOffset, dataOffset + dataLength)
      .map { c =>
        if (
          ((c >= 0x00) && (c <= 0x1f)) || (c == 0x7f) || ((c >= 0x80.toByte) && (c <= 0x9f.toByte))
        ) 0x2e.asInstanceOf[Byte]
        else c
      }
      .toArray
    // Decode to ASCII
    ...
    val ascii = asciiSectionsWithSize
      .map { case (section, sectionSize) =>
        ensureLength(
          decoder.decode(ByteBuffer.wrap(section)).toString,
          sectionSize
        )
      }
      .mkString("|", "|", "|")
  ...
  def dump(
    ...
    loop(
      dataOffset,
      if (dataLength < 0) data.length - dataOffset else dataLength
    )
    if (end) done()
  }
...
object HexDumper {
  ...
  protected def parseParams(args: Array[String]): Params = {
    val parser = new scopt.OptionParser[Params](getClass.getSimpleName) {
      ...
      opt[String]("view-hex-mode").text("Hexadecimal view mode").action {
        (v, c) =>
      ...
      opt[Int]("view-bytes").text("How many bytes to display per line").action {
        (v, c) =>
          c.copy(viewBytes = v)
      }
      checkConfig { c =>
  ...
  def dump(data: Array[Byte], settings: Settings): Unit = {
    ...
    dumper.dump(
      data,
      actualSettings.offset.toInt,
      actualSettings.length.toInt,
      end = true
    )
  }
  ...
  object Output {
    ...
    def apply(out: StringBuilder): ScalaStringBuilderOutput =
      new ScalaStringBuilderOutput(out)
