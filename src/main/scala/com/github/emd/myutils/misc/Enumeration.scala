...
trait EnumerationWithAliases extends scala.Enumeration {
  ...
  protected class ValWithAliases(i: Int, name: String, val aliases: Set[String])
      extends Val(i, name) {
    def this(name: String, aliases: Seq[String]) =
      this(nextId, name, aliases.toSet)
  ...
  def byName(s: String): Value = {
    values
      .find {
        case withAliases: ValWithAliases =>
          withAliases.normalized.contains(normalize(s))
