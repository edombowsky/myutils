...
class SimplePathFilter(filter: FileFilter) extends PathFilter {
...
class ChildPathFilter(child: String) extends PathFilter {
...
class ChildrenPathFilter(
  ...
  override def search(base: File): Set[File] =
    if (Files.isDirectory(base.toPath)) {
      ...
      Files.isSymbolicLink(base.toPath) || Files.isRegularFile(base.toPath)
    ) {
      Set(base)
    } else {
      Set[File]()
    ...
    (files ::: {
      if (levels > 0) {
        ...
        for {
          ...
          child <- searchDescendants(
            new File(base, childDirectory.getName),
            levels - 1
          )
        } yield child
      } else Nil
