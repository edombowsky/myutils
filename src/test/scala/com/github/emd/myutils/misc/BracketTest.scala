...
class BracketTest extends AnyFunSuite with Matchers {

  test(
    "Bracket.auto should be successful if everything goes well and the resource should be closed"
  ) {
  ...
  test(
    "Bracket.auto should be successful if everything goes well, even if the resource fails closing"
  ) {
    val expectedResult = 111
    var closedFlag: Boolean = false
  ...
  test(
    "Bracket.auto should fail if the code fails and the resource should be closed"
  ) {
    var closedFlag: Boolean = false
    trait Resource extends AutoCloseable {
      override def close(): Unit = closedFlag = true
